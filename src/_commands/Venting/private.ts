  ...
  run: async (tvf, msg, args, { prefix }) => {
    ...
    // Handle the cancellation of a session
      ...
      if (id && (tvf.isUser('Support', msg.member) || tvf.isUser('Admin', msg.member))) {
        ...
        const cancelled = tvf.createEmbed({ colour: tvf.const.colours.red, timestamp: true, thumbnail: false, author: true }, msg)
        ...
        tvf.const.channels.staff.support.send(cancelled);
        tvf.const.channels.staff.private.logs.send(cancelled);

        // Send the user a message informing them that their session has been cancelled
        venter.send(
          tvf.createEmbed({ colour: tvf.const.colours.red, timestamp: true })
        ...
        ).catch(() => tvf.const.channels.community.discussion.send(stripIndents`
      ...
      // If the user wants to cancel their own session
        ...
        if (!doc.private.requested) return msg.author.send(tvf.emojiMessage(tvf.const.emojis.cross, 'Sorry, you do not have a pending private venting session to cancel!'));
    ...
    // Handle the requesting of a session
      ...
      if (!reason) return msg.author.send(tvf.emojiMessage(tvf.const.emojis.cross, 'You must provide a reason for your session!'));

      // Ensure that the user hasn't already got a pending session
      if (doc.private.requested) return msg.author.send(tvf.emojiMessage(tvf.const.emojis.cross, `You already have a pending session! If you would like to request a new one, please first cancel your pending session by running \`${prefix}private cancel\`!`));
      ...
      const sessionRequested = tvf.createEmbed({ colour: tvf.const.colours.green, timestamp: true, thumbnail: false, author: true }, msg)
      ...
      tvf.const.channels.staff.support.send(tvf.isProduction ? tvf.const.roles.staff.support.toString() : '', sessionRequested);
      tvf.const.channels.staff.private.logs.send(sessionRequested);

      // Send the user a message confirming that their session has been requested
      msg.author.send(
        tvf.createEmbed({ colour: tvf.const.colours.green, timestamp: true, author: true }, msg)
        ...
        timeout.timeout(doc.private.id, tvf.privateTimeout, () => {
          ...
          tvf.const.channels.staff.support.send(
            tvf.createEmbed({ colour: tvf.const.colours.red, timestamp: true, thumbnail: false, author: true }, msg)
          ...
          msg.author.send(
            tvf.createEmbed({ colour: tvf.const.colours.red, timestamp: true, author: true }, msg)
        ...
        const reminderEmbed = tvf.createEmbed({ colour: tvf.const.colours.orange, timestamp: true, thumbnail: false, author: true }, msg)
          .setThumbnail(msg.author.avatarURL())
          .setDescription(`Reason: ${reason}`)
          .addFields([
            { name: 'Session ID', value: doc.private.id, inline: true },
        ...
        timeout.timeout(`${doc.private.id}1`, hour, () => tvf.const.channels.staff.support.send(reminderEmbed.setTitle(`${msg.author.username}'s session will expire in five hours!`)));
        timeout.timeout(`${doc.private.id}2`, hour * 2, () => tvf.const.channels.staff.support.send(reminderEmbed.setTitle(`${msg.author.username}'s session will expire in four hours!`)));
        timeout.timeout(`${doc.private.id}3`, hour * 3, () => tvf.const.channels.staff.support.send(reminderEmbed.setTitle(`${msg.author.username}'s session will expire in three hours!`)));
        timeout.timeout(`${doc.private.id}4`, hour * 4, () => tvf.const.channels.staff.support.send(reminderEmbed.setTitle(`${msg.author.username}'s session will expire in two hours!`)));
        timeout.timeout(`${doc.private.id}5`, hour * 5, () => tvf.const.channels.staff.support.send(tvf.isProduction ? tvf.const.roles.staff.support.toString() : '', reminderEmbed.setTitle(`${msg.author.username}'s session will expire in one hour!`)));
    }
  }
} as Command;
