...
export default {
	...
	run: async (tvf, msg, args) => {
        ...
        const embed = tvf.createEmbed({ timestamp: true, colour: tvf.const.colours.red })
            ...
            .addField(`Denied by ${msg.author.username}`, `**${tvf.const.emojis.suggestions.downvote.toString()}  |**  ${_.truncate(comment, { length: tvf.embedLimit.field.value - 20 })}`)
        ...
        // notify the user
        const denied = tvf.createEmbed({ colour: tvf.const.colours.red, timestamp: true })
