...
class Staff extends TVFCommand {
    constructor() {
        super('staff', {
            aliases: ['staff'],
            category: 'Core',
            description:
                'Allows you to view stats about our wonderful staff! <3',
            args: [
                {
                    id: 'position',
                    type: 'lowercase',
        ...
        this.examples = ['staff', 'staff Admin', 'staff Forest Keepers'];
    }
    ...
    exec(msg: Message, { position }: { position: string }) {
        ...
        support = support
            .filter((m) => !support.intersect(admins).has(m.id))
            .filter((m) => !moderators.intersect(support).has(m.id));
        moderators = moderators.filter(
            (m) => !moderators.intersect(admins).has(m.id),
        );

        const fullList = this.client.utils
            .embed()
            ...
            .addField(
                `Forest Keepers (${moderators.size})`,
                moderators.map((m) => m.user),
                true,
            )
            .addField(
                `Support (${support.size})`,
                support.map((s) => s.user),
                true,
            )
            .addField(
                'Online',
                staff.filter((s) => s.user.presence.status !== 'offline').size,
            );
            ...
            switch (pluralise(position)) {
                ...
                case 'admins':
                    msg.channel.send(
                        this.generateEmbed(
                            admins,
                            this.client.constants.Colours.Yellow,
                            'Administrators',
                        ),
                    );
                    break;
                ...
                case 'supports':
                    msg.channel.send(
                        this.generateEmbed(
                            support,
                            this.client.tvfRoles.staff.support.hexColor,
                            'Support',
                        ),
                    );
                    break;
                ...
                case 'fks':
                    msg.channel.send(
                        this.generateEmbed(
                            moderators,
                            this.client.tvfRoles.staff.moderators.hexColor,
                            'Forest Keepers',
                        ),
                    );
                    break;
