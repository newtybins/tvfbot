...
export default {
	...
	run: async (tvf, msg, args) => {
        ...
        const embed = tvf.createEmbed({ timestamp: true, colour: tvf.const.colours.green })
            ...
            .addField(`Approved by ${msg.author.username}`, `**${tvf.const.emojis.suggestions.upvote.toString()}  |**  ${_.truncate(comment, { length: tvf.embedLimit.field.value - 20 })}`)
        ...
        // notify the user
        const approved = tvf.createEmbed({ colour: tvf.const.colours.green, timestamp: true })
