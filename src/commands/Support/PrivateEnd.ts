...
class PrivateEnd extends TVFCommand {
    constructor() {
        super('privateEnd', {
            aliases: ['privateVent-end', 'pve'],
            description:
                'Allows members of staff to end a privateVent venting session!',
            args: [
                {
                    id: 'id',
                    type: 'string',
                    ...
                    prompt: {
                        start: (msg: Message): string =>
                            `${msg.author}, what is the ID of the session you would like to end?`,
                    },
                },
                {
                    id: 'notes',
    ...
    async endSession(privateVent: Private, staff: User, notes: string) {
        ...
        const text = this.client.server.channels.cache.get(
            privateVent.textID,
        ) as TextChannel;
        const voice = this.client.server.channels.cache.get(
            privateVent.voiceID,
        ) as VoiceChannel;
        const startedAt = moment(privateVent.startedAt).format(
            this.client.constants.moment,
        );
        ...
        const paste = await this.client.pastebin.pastes.create(
            stripIndents`
            ...
            ${messages
                .map(
                    (msg) =>
                        `${moment(msg.createdTimestamp).format(
                            'D/M/YYYY HH:MM',
        ...
        const embed = this.client.utils
            ...
            .addField(
                'Open for',
                ms(moment().diff(moment(privateVent.startedAt), 'ms'), {
                    long: true,
                }),
            )
        ...
        if (paste.url) {
            ...
            embed.addField(
                'Pastebin',
                paste.url ? paste.url : 'Max daily paste upload limit met ):',
                true,
            );
        }
        ...
        this.client.logger.command(
            `${this.client.userLogCompiler(
                staff,
            )} just ended ${this.client.userLogCompiler(
                user,
            )}'s privateVent venting session (${privateVent.id})`,
        );
    }

    async exec(msg: Message, { id, notes }: { id: string; notes: string }) {
        this.client.utils.deletePrompts(msg); // Delete any prompts
        const privateVent = await this.client.db.private.findUnique({
            where: { id },
        ...
        if (!privateVent) {
            const error = this.client.utils
                ...
                .setTitle(
                    'There was an error trying to end that privateVent venting session!',
                )
                .setDescription(
                    `An active privateVent venting session could not be found with the ID \`${id}\`. Please check that you have entered it exactly as shown in the request, and try again (IDs are cAsE sensitive!)`,
                );
