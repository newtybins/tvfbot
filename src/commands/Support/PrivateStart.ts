...
class PrivateStart extends TVFCommand {
    constructor() {
        super('privateStart', {
            aliases: ['private-start', 'pvs'],
            description:
                'Allows members of staff to start a private venting session!',
            args: [
                {
                    id: 'id',
                    type: 'string',
                    ...
                    prompt: {
                        start: (msg: Message): string =>
                            `${msg.author}, what is the ID of the session you would like to start?`,
                    },
                },
            ],
        });
        ...
        this.examples = ['private-start', 'private-start ci1de'];
    }
    ...
    async exec(msg: Message, { id }: { id: string }) {
        ...
        if (!privateVent) {
            const error = this.client.utils
                ...
                .setTitle(
                    'There was an error trying to start that private venting session!',
                )
                .setDescription(
                    `An active private venting session could not be found with the ID \`${id}\`. Please check that you have entered it exactly as shown in the request, and try again (IDs are cAsE sensitive!)`,
                );
        ...
        const text = await this.client.server.channels.create(
            `${user.username}-${user.discriminator}`,
            {
                parent: this.client.tvfChannels.staff.private.category,
                type: 'text',
                topic: `${
                    this.client.constants.Emojis.Tick
                } | Session started: ${moment(startedAt).format(
        ...
        embed
            .setTitle(
                `Welcome to your private venting session, ${user.username}!`,
            )
            .addField('Session opened by', msg.author.username)
            .addField(
                'Session started',
                moment(startedAt).format(this.client.constants.moment),
            )
        ...
        this.client.logger.command(
            `${this.client.userLogCompiler(
                msg.author,
            )} just started ${this.client.userLogCompiler(
                user,
