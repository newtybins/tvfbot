const staff: Command = {
  run: (client, msg, args) => {
    ...
    const admins = msg.guild.roles
      ...
      .members.concat(
        msg.guild.roles.find(r => r.id === client.config.roles.techAdmin)
          .members
      );
    const mods = msg.guild.roles.find(r => r.id === client.config.roles.mod)
      .members;
    ...
    if (
      ...
      const embed = client
        ...
        .addField(
          "Online",
          fks.filter(fk => fk.user.presence.status !== "offline").size,
          true
        );
    ...
    if (
      ...
      const embed = client
        ...
        .addField(
          "Online",
          mods.filter(mod => mod.user.presence.status !== "offline").size,
          true
        );
    ...
    if (
      ...
      const embed = client
        ...
        .addField(
          "Online",
          admins.filter(admin => admin.user.presence.status !== "offline").size,
          true
        );
      ...
      const embed = client
        .createEmbed("purple")
        .setTitle("Staff")
        .setDescription(`Total: **${admins.size + mods.size + fks.size}**`)
        .addField(
