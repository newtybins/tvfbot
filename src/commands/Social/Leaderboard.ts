...
class Leaderboard extends Command {
	...
	async exec(msg: Message, { page }: { page: number }) {
        ...
        // Generate field data
        const fields: EmbedFieldData[] = await Promise.all(filteredUsers.slice((page * 10) - 10, page * 10).map(async (user) => {
        ...
        const xp = (user.xp - this.client.social.xpFor(user.level)).toLocaleString();
