...
class Help extends TVFCommand {
    constructor() {
        super('help', {
            aliases: ['help'],
            category: 'Core',
            description:
                'Get either a list of the available commands or instructions for specified commands! Specifying a command is opitional. If a command was specified, its help text will show up.',
            args: [
                {
                    id: 'commandName',
                    type: 'string',
    ...
    exec(msg: Message, { commandName }: { commandName: string }) {
        ...
        if (!commandName) {
            ...
            embed
                ...
                .setDescription(
                    `For detailed information run \`${this.handler.prefix}help <commandName>\``,
                );
            ...
            if (command) {
                try {
                    ...
                    this.client.logger.command(
                        `${this.client.userLogCompiler(
                            msg.author,
                        )} requested information about the ${commandName} command!`,
                    );
                } catch {
                    embed.fields = [];
                    embed
                        ...
                        .setDescription(
                            "Please let newt know and they'll get right to fixing it!",
                        )
                        .setColor(this.client.constants.Colours.Red);

                    this.client.logger.error(
                        `${commandName}'s help object is incomplete - woops!`,
                    );
                }
            } else {
                embed
                    ...
                    .setDescription(
                        `Sorry, but I could not find any information about the command \`${commandName}\``,
                    )
                    .setColor(this.client.constants.Colours.Red);
            }
        }
