...
export default class TVFSocial {
    ...
    levelReward(level: number): LevelReward {
        const levelIndex = this.client.constants.levelRoles.findIndex((l) =>
    ...
    async updateSuggestionStatus(
        ...
        let suggestion = await this.client.db.suggestion.findUnique({
            where: { id: suggestionID },
        });
        const suggester = this.client.users.cache.get(suggestion.authorID);
        const message =
            await this.client.tvfChannels.community.suggestions.messages.fetch(
        ...
        switch (suggestion.status) {
            case this.client.constants.SuggestionStatus.Approved:
                embed
                    ...
                    .setTitle(
                        `Suggestion by ${suggester.username} has been approved!`,
                    );
            ...
            case this.client.constants.SuggestionStatus.Denied:
                embed
                    ...
                    .setTitle(
                        `Suggestion by ${suggester.username} has been denied!`,
                    );
                suggesterNotification
                    .setColor(this.client.constants.Colours.Red)
            ...
            case this.client.constants.SuggestionStatus.Considered:
                embed
                    ...
                    .setTitle(
                        `Suggestion by ${suggester.username} has been considered!`,
                    );
                suggesterNotification
                    .setColor(this.client.constants.Colours.Yellow)
        ...
        suggester.send(suggesterNotification);
    }
}
