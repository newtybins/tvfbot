...
class VCLink extends Listener {
    ...
    leaveChannel(id: string, state: VoiceState) {
        ...
        const oldChannel = this.client.server.channels.cache.get(
            this.client.constants.vcLink[id],
        ) as TextChannel;
    ...
    exec(oldState: VoiceState, newState: VoiceState) {
        if (this.client.production) {
            ...
            if (
                Object.keys(this.client.constants.vcLink).includes(
                    newChannel,
                ) ||
                Object.keys(this.client.constants.vcLink).includes(oldChannel)
            ) {
                // If there is a link for the VC the user has joined
                // If the user has joined the voice channel
                if (
                    oldChannel !== newChannel &&
                    typeof newChannel === 'string'
                ) {
                    const newC = this.client.server.channels.cache.get(
                        this.client.constants.vcLink[newChannel],
                    ) as TextChannel; // Get the associated text channel
                    newC.updateOverwrite(newState.member.id, {
                        VIEW_CHANNEL: true,
                        SEND_MESSAGES: true,
                        READ_MESSAGE_HISTORY: true,
                        ADD_REACTIONS: true,
                    }); // Let the user speak in the channel
                    newC.send(
                        `${newState.member} joined ${newState.channel.name}! Say hi (:`,
                    );
