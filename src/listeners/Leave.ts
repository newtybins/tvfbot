...
class Leave extends Listener {
    ...
    async exec(member: GuildMember) {
        if (this.client.production) {
            ...
            // Save sticky roles
            const staffRoles = Object.entries(this.client.tvfRoles.staff).map(
                (r) => r[1].id,
            );
            ...
            if (privateVent) {
                ...
                if (privateVent.startedAt && privateVent.startedAt !== null) {
                    PrivateEnd.prototype.endSession(
                        privateVent,
                        this.client.user,
            ...
            await this.client.db.user.update({
                where: { id: user.id },
                data: {
                    stickyRoles: member.roles.cache
                        .map((r) => r.id)
                        .filter(
                            (r) =>
                                ![
                                    ...staffRoles,
                                    this.client.server.roles.everyone,
                                ].includes(r),
                        ),
                },
            });
            ...
            this.client.tvfChannels.general.send(
                `**${member.user.tag}** has exited the Forest.`,
            );
        }
    }
}
