...
class Join extends Listener {
    ...
    async exec(member: GuildMember) {
        if (this.client.production) {
            ...
            if (this.client.botBanner && member.user.bot)
                ...
                if (!userRow) {
                    this.client.logger.info(
                        `${member.user.tag} joined the server for the first time!`,
                    );
                    hereBefore = false;
                    ...
                    this.client.logger.info(
                        `${member.user.tag} rejoined the server!`,
                    );

                    // If there is sticky roles, add them back
                    if (userRow.stickyRoles && userRow.stickyRoles.length > 0) {
                        userRow.stickyRoles.forEach((r) =>
                            member.roles.add(r, 'Adding back sticky roles...'),
                        );
            ...
            const welcome = this.client.utils
                .embed()
            ...
            if (hereBefore)
                welcome.setTitle(
                    `Welcome back to TVF, ${member.user.username}!`,
                );
            else welcome.setTitle(`Welcome to TVF, ${member.user.username}!`);

            const msg = await this.client.tvfChannels.general.send(
                `**Welcome ${hereBefore ? 'back' : ''} to TVF, <@!${
                    member.id
                }>!** ${
                    ...
                    member.user.bot
                        ? ''
                        : `(${this.client.tvfRoles.community.welcomeTeam.toString()})`
                }`,
                welcome,
            );
