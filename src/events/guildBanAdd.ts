...
export default async (tvf: Client, _guild: Discord.Guild, user: Discord.User) => {
	if (tvf.isProduction) {
    	...
    	if (tvf.config.botbanner && user.bot) {
			const general = tvf.createEmbed({ colour: tvf.const.colours.red, timestamp: true, thumbnail: false, author: false })
				.setThumbnail(user.avatarURL())
				.setTitle('Begone Bot!')
				.setDescription(`${user.tag} was banned from the server! Stinky bot!`);

			const staff = tvf.createEmbed({ colour: tvf.const.colours.red, timestamp: true, thumbnail: false, author: false })
				.setThumbnail(user.avatarURL())
				.setTitle('Begone Bot!')
			...
			tvf.const.channels.general.send(general);
			tvf.const.channels.staff.moderators.chat.send(staff);
		} else {
			tvf.const.channels.general.send(`**${user.tag}** has been banned from the Forest.`);
		}
	}
};
