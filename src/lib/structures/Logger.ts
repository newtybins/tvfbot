...
export default class Logger {
    ...
    constructor(client: Client, scope = 'TVF Bot') {
        this.client = client;
        this.scope = scope.toLowerCase();
    }

    private async parseMessage(message: string, placeholders: any[]) {
        const userMentions = message.match(MessageMentions.USERS_PATTERN);
        const channelMentions = message.match(MessageMentions.CHANNELS_PATTERN);
        ...
        if (userMentions?.length > 0) {
            await Bluebird.map(userMentions, async mention => {
                ...
                const id = mention.replace(extractNumbers, '');
    ...
    public loader(message: string, ...placeholders: any[]) {
        this.log('loader', 'green', message, placeholders);
    }
