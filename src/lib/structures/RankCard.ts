...
class RankCard {
    ...
    constructor(input) {
        ...
        if (input.fonts && input.fonts.length > 0) {
            ...
            // Regular & Bold
            Canvas.registerFont(path.resolve(assetsDir, 'fonts', 'neutra.otf'), {
                family: 'Neutra'
            });
        }
        ...
        this._data = {
            ...
            status: {
                ...
                colour: STATUS_COLOURS[member.presence?.status ?? 'offline'],
                circle: false
            },
            overlay: {
                display: true,
    ...
    async build(fonts = { regular: 'Neutra', bold: 'Neutra' }) {
        ...
        const progressText = `${abbreviate(this._data.progress.currentXP)}/${abbreviate(
            this._data.progress.requiredXP
        )}`;
        ...
        if (this._data.avatar.display) {
            ...
            ctx.beginPath();
            ctx.arc(
                this._data.avatar.x + this._data.avatar.radius,
                this._data.avatar.y + this._data.avatar.radius,
                this._data.avatar.radius,
                0,
                Math.PI * 2,
                true
            );
            ...
            ctx.drawImage(
                ...
                this._data.avatar.x,
                this._data.avatar.y,
                this._data.avatar.width + 20,
                this._data.avatar.height + 20
            );
            ctx.restore();
        }
        ...
        if (this._data.progress.display) {
            ...
            if (this._data.progress.bar.rounded) {
                ...
                ctx.arc(
                    this._data.progress.x + this._data.progress.bar.radius,
                    this._data.progress.y +
                        this._data.progress.bar.radius +
                        (this._data.progress.height - 1.25),
                    this._data.progress.bar.radius,
                    1.5 * Math.PI,
                    0.5 * Math.PI,
                    true
                ...
                ctx.fillRect(
                    this._data.progress.x + this._data.progress.bar.radius,
                    this._data.progress.y + (this._data.progress.height - 1.25),
                    1.03101424979 * this._data.progress.width - this._data.progress.bar.radius,
                    this._data.progress.height
                );
                ctx.arc(
                    this._data.progress.x + 1.03101424979 * this._data.progress.width,
                    this._data.progress.y +
                        this._data.progress.bar.radius +
                        (this._data.progress.height - 1.25),
                    this._data.progress.bar.radius + 0.25,
                    1.5 * Math.PI,
                    0.5 * Math.PI,
                    false
                );
        ...
        if (this._data.level.display) {
            ...
            ctx.fillText(
                `Level ${abbreviate(this._data.level.value)}`,
                this._data.progress.x + 10,
    ...
    setProgressBar(
        colour,
        trackColour = this._data.progress.bar.trackColour,
        rounded = this._data.progress.bar.rounded
    ) {
